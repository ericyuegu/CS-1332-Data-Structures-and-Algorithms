...
public class StringSearching {
    ...
    public static int generateHash(CharSequence current, int length) {
        if (current == null)    {
            ...
            throw new IllegalArgumentException("Length must be positive and "
                    + "no greater than current length");
        }

        int sum = 0;
        for (int i = 0; i < length; i++)    {
            sum += (int) current.charAt(i) * pow(BASE, length - 1 - i);
        }
    ...
    public static int updateHash(int oldHash, int length, char oldChar,
                                 char newChar) {
        return BASE * (oldHash - (int) oldChar * pow(BASE, length - 1))
                + (int) newChar;
    }
